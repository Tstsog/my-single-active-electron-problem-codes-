...
% for lithium (he) atom using a single-active electron (sae) approximation Ref.[1], 
...
%
[En_1s, En_2s, En_3s, rho_1s, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, b1, b2, 0., 1); % s orbital
[En_1s, En_2s, En_3s, rho_2s, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, b1, b2, 0., 2); % s orbital
[En_2p, En_3p, En_4p, rho_2p, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, b1, b2, 1., 1); % p orbital
%
rho_total = 2.*rho_1s + 1.*rho_2s + 0.*rho_2p;
%
...
[En_1s, En_2s, En_2p] % -2.026949837424999  -0.200199535083089  -0.131413894537176 vs 
%                     %                     -0.200                                 from Ref.[1]   
%
average_r2 = sum(wr.*rho_total.*r.^2.*r.^2.*4.*pi) % 17.948212300791209
...
%
%%%
function [En_1, En_2, En_3, rho, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, b1, b2, ell, n_ind)
%
[r,wr,D]=legDC2(N,a,b);
%
wr = wr(2:N);
...
D2 = D2(2:N,2:N);
%
V_sae = -(C0./r) - (Zc.*exp(-c.*r)./r) - (a1.*exp(-b1.*r) + a2.*exp(-b2.*r));
%
