...
% for argon (ar) atom using a single-active electron (sae) approximation Ref.[1], 
...
b = 20.;     % end point of cooridnate r; you may change it
%
[En_1s, En_2s, En_3s, rho_1s, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 0., 1); % s orbital
[En_1s, En_2s, En_3s, rho_2s, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 0., 2); % s orbital
[En_1s, En_2s, En_3s, rho_3s, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 0., 3); % s orbital
[En_2p, En_3p, En_4p, rho_2p, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 1., 1); % p orbital
[En_2p, En_3p, En_4p, rho_3p, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 1., 2); % p orbital
[En_3d, En_4d, En_5d, rho_3d, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, 2., 1); % d orbital
%
rho_total = 2.*rho_1s + 2.*rho_2s + 6.*rho_2p + 2.*rho_3s + 6.*rho_3p;
%
...
[En_1s, En_2s, En_2p, En_3s, En_3p] %  
% [En_1s, En_2s, En_2p, En_3s, En_3p]
% -114.7979  -11.4390   -9.0797   -1.1220   -0.6068
%                                           -0.607  % from Ref.[1]     
%%%
[ En_4p, En_3d] % 
% [ En_4p, En_3d]
% -0.1084   -0.0715  
% -0.10843  -0.07170   from Ref.[1]
%
average_r2 = sum(wr.*rho_total.*r.^2.*r.^2.*4.*pi) % 24.6703
...
%
%%%
function [En_1, En_2, En_3, rho, wr, r] = ne_sae_ham(N,a,b, C0, Zc, c, a1, a2, a3, b1, b2, b3, ell, n_ind)
%
[r,wr,D]=legDC2(N,a,b);
%
wr = wr(2:N);
...
D2 = D2(2:N,2:N);
%
V_sae = -(C0./r) - (Zc.*exp(-c.*r)./r) - (a1.*exp(-b1.*r) + a2.*exp(-b2.*r) + a3.*exp(-b3.*r));
%
